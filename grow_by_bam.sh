...
for file in $dir/*; do
  ...
  ## --- Left forward reads ---
  samtools view -h -F 2324 $bam "${contig[@]}" | # filter flags UNMAP,SECONDARY,SUPPLEMENTARY,REVERSE
  egrep '^@|[0-9]{1,3}S[0-9]{1,3}=' | # select soft-clipped at the left edge by CIGAR
  samtools fasta > $file.fl.fasta
  
  ## --- Right forward reads ---
  samtools view -h -F 2324 $bam "${contig[@]}" | # filter flags UNMAP,SECONDARY,SUPPLEMENTARY,REVERSE
  egrep '^@|[0-9]{1,3}=[0-9]{1,3}S' | # select soft-clipped at the right edge by CIGAR
  samtools fasta > $file.fr.fasta
  
  # === Reverse reads ===
  echo "Processing reverse reads ..."
  ## --- Left reverse reads ---
  samtools view -h -F 2308 -f 16 $bam "${contig[@]}" | # filter flags UNMAP,SECONDARY,SUPPLEMENTARY
  egrep '^@|[0-9]{1,3}S[0-9]{1,3}=' | # select soft-clipped at the left edge by CIGAR
  samtools fasta |
